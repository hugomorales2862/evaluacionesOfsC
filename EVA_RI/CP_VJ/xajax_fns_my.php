...
function Grabar_evaluacion1($cod_eva,$tipo_evaluacion,$preguntas,$usuario,$obs_inmediato,$obs_final,$obs){
		...
		if($tipo_evaluacion == 20){
			...
			for($i = 21; $i <= 40; $i++){
				...
				$preg = $preguntas[$i];
				if($i <= 25){
					$sql.= $ClsIng->inserta_notas(1,$cod_eva,$j,$preg,6,$fecha,$usuario);
				}else if($i >25 and  $i <= 30){
					$sql.= $ClsIng->inserta_notas(2,$cod_eva,$j,$preg,6,$fecha,$usuario);
				}else if($i >30 and  $i <= 35){
					$sql.= $ClsIng->inserta_notas(3,$cod_eva,$j,$preg,6,$fecha,$usuario);
				}else if($i >35 and  $i <= 40){
					$sql.= $ClsIng->inserta_notas(4,$cod_eva,$j,$preg,6,$fecha,$usuario);
				}
			}
	...
	function Grabar_evaluacion2($cod_eva,$tipo_evaluacion,$preguntas,$usuario,$obs_inmediato,$obs_final,$obs){
		$respuesta = new xajaxResponse();
		$ClsIng = new ClsIngreso();
		date_default_timezone_set('America/Guatemala');
		$dias= date("d");
