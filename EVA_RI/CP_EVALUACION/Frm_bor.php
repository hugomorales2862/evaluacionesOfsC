...
include_once("xajax_fns_my.php");
	...
	date_default_timezone_set('America/Guatemala');
		...
		$annio = date("Y");
		$siguiente = $annio + 1;
	//se agrego evaluacion meses octubre y noviembre para generar la eva comtes jefes y
	//	if ($mes == 6 or $mes == 7 or $mes == 8 or $mes == 9 ){
		if ($mes == 6 or $mes == 7 or $mes == 8 or $mes == 9 or $mes == 10){
			$evaluacion = 1;
			$comp_eva = '1 - '.$annio;
		//}else if($mes == 12 or $mes == 11 or $mes == 10){
		}else if($mes == 12) {
			$evaluacion = 2;
			$comp_eva = '2 - '.$annio;
		}
		...
		if($total_sol > 0){
			echo'<script>
				alert("ESTE CATALOGO YA TIENE BORRADOR REGISTRADO PARA EL PsssdfdfERIODO '.$comp_eva.'" );
				history.back()
				</script>';
			exit();
		}
...
xajax_Grabar_borrador(evaluacion,linea,destino,autoevaluado,inmediato,eva_final,codigo_arma1,codigo_grado1,empleo1,tiempo1,codigo_arma2,codigo_grado2,empleo2,tiempo2,codigo_arma3,codigo_grado3,empleo3,tiempo3,puesto_ant,dep,renglon,tipo_evaluacion);
	...
	<form accept-charset="UTF-8">
		...
		<input type='hidden' name='tipo_evaluacion' id='tipo_evaluacion' value='0'></input> <!--el TIPO 0 == borrador, 1 es autoevaluacion, 2 = inmediato, 3 = final-->
		...
		<table width='100%' border='1' >
			...
			<tr>
			<td colspan='2'><center><strong><small>EMPLEO ACTUAL Y UNIDAD<br><br></small></strong><input type='text' class='span5' id='empleo1' placeholder='Empleo' value='<?php echo $empleo; ?>' onkeyup='mayusculas(this);'></input></center></td>
			...
			<td><center><strong><small>EMPLEO ANTERIOR<br><br></small></strong><input type='text' class='span3' id='puesto_ant' placeholder='Empleo' value='<?php if ($puesto_ant != ""){ echo $puesto_ant; }else{ $puesto_ant = "DESCONOCIDO"; echo $puesto_ant;}?>' onkeyup='mayusculas(this);'></input></center></td>
