...
function Trae_Datos_Catalogo_inmediato($cat){
		...
		if(is_array($result)){
			...
			foreach($result as $row){
				...
				$nombre = trim($row["PER_APE1"].''.$row["PER_APE2"].', '.$row["PER_NOM1"].''.$row["PER_NOM2"]);
			...
			// 	$t_puesto_inmediato = $years . $months . $days;
			// }

			// $respuesta->alert($t_puesto_inmediato);

			// if ($t_puesto_inmediato >= 300) {
			...
			// 	}
			// }else{
			// 	$respuesta->alert("NO CUENTA CON EL TIEMPO MINIMO PARA EVALUAR");
			// 	$respuesta->assign("nombre_inmediato","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
			// 		$respuesta->assign("empleo2","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
			// 		$respuesta->assign("tiempo2","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
			// 		$respuesta->assign("codigo_grado2","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
			// 		$respuesta->assign("codigo_arma2","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
	...
	function Trae_Datos_Catalogo_comte($cat){
		...
		// }else{
		// 		$respuesta->alert("NO CUENTA CON EL TIEMPO MINIMO PARA EVALUAR");
		// 	$respuesta->assign("nombre_comte","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
		// 		$respuesta->assign("empleo3","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
		// 		$respuesta->assign("tiempo3","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
		// 		$respuesta->assign("codigo_grado3","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
		// 		$respuesta->assign("codigo_arma3","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
		// }
		}else{
			$respuesta->alert("Catalogo invalido");	// si no trae datos levanto una alerta.
			$respuesta->assign("per_nombre","value",'');	// le asigno el contenido a respuesta para insertarlo en el elemento
	...
	$puesto_ant,$dep,$renglon,$tipo_evaluacion,$tipo_oficial){
		$respuesta = new xajaxResponse();
		$ClsIng = new ClsIngreso();
		$ClsPer = new ClsPersonal();
		//$respuesta->alert($tipo_oficial);
		if ($tipo_oficial == 3){
			$tipo_evaluacion = 3;
		}else{
	...
	function Grabar_evaluacion1($cod_eva,$tipo_evaluacion,$preguntas,$usuario,$obs_inmediato,$obs_final,$obs,$autoevaluado){
		...
		$annio = date("Y");
		$fecha = $annio."/".$mes."/".$dias;
		if($tipo_evaluacion==17 or $tipo_evaluacion==21 or $tipo_evaluacion==25){
	...
	function Grabar_evaluacion2($cod_eva,$tipo_evaluacion,$preguntas,$usuario,$obs_inmediato,$obs_final,$obs,$autoevaluado){
		$respuesta = new xajaxResponse();
		$ClsIng = new ClsIngreso();
		date_default_timezone_set('America/Guatemala');
		$dias= date("d");
		...
		$annio = date("Y");
		$fecha = $annio."/".$mes."/".$dias;
		$total_eva = $ClsIng->verifica_notas1($cod_eva,5,$autoevaluado);
		//$respuesta->alert($total_eva);
		//return $respuesta;
		...
		$sql = $ClsIng->update_evaluacion($cod_eva,$tipo_evaluacion,$obs_inmediato,$obs_final,$obs);
			for($i = 1; $i <= 20; $i++){
				$preg = $preguntas[$i];
				if($i <= 5){
					$sql.= $ClsIng->inserta_notas(1,$cod_eva,$i,$preg,5,$fecha,$usuario);
				}else if($i >5 and  $i <= 10){
					$sql.= $ClsIng->inserta_notas(2,$cod_eva,$i,$preg,5,$fecha,$usuario);
				}else if($i >10 and  $i <= 15){
					$sql.= $ClsIng->inserta_notas(3,$cod_eva,$i,$preg,5,$fecha,$usuario);
				}else if($i >15 and  $i <= 20){
					$sql.= $ClsIng->inserta_notas(4,$cod_eva,$i,$preg,5,$fecha,$usuario);
				}		
			}
		}else{
			$respuesta->alert("ESTA EVALUACION YA TIENE NOTAS INGRESADAS EN EL SISTEMA");
	...
	$xajax->register(XAJAX_FUNCTION, "Grabar_evaluacion2");
