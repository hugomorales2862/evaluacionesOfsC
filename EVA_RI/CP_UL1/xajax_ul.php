...
// ==================================================================================================================================================-->
	...
	function Anulacion_evaluacion($cod_eva){
		...
		$ClsIng = new ClsIngreso();
		if($cod_eva != ""){
			$sql.= $ClsIng->anulacion_evaluacion($cod_eva);
			$rs = $ClsIng->exec_sql($sql);
			// $respuesta->alert($rs);
			if($rs == 1){
				$respuesta->call('xajax_Alert_Eliminado', 1);
	...
	function Impugnar_evaluacion($cod_eva){
		...
		$ClsIng = new ClsIngreso();
		if($cod_eva != ""){
			$sql.= $ClsIng->impugnar_evaluacion($cod_eva);
			$rs = $ClsIng->exec_sql($sql);
			// $respuesta->alert($rs);
			if($rs == 1){
				$respuesta->call('xajax_Alert_Eliminado', 2);
				$respuesta->script("$('#alert_modal').modal({backdrop:'static',keyboard:false, show:true});$('#alert_modal').modal('show')");
			}else{
				$respuesta->alert("OCURRIO UN ERROR, INTENTE MAS TARDE");
			}
...
// ==================================================================================================================================================-->
	...
	$xajax->register(XAJAX_FUNCTION, "Anulacion_evaluacion");
	$xajax->register(XAJAX_FUNCTION, "Impugnar_evaluacion");
