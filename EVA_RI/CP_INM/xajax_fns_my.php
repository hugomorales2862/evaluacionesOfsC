...
$xajax->configure('decodeUTF8Input',true);  // para corregir 
	...
	function Grabar_evaluacion($cod_eva,$tipo_evaluacion,$preguntas,$usuario,$obs_inmediato,$obs_final,$obs){
		...
		$sql = $ClsIng->update_evaluacion($cod_eva,$tipo_evaluacion,$obs_inmediato,$obs_final,$obs);
			for($i = 1; $i <= 20; $i++){
				$preg = $preguntas[$i];
				if($i <= 5){
					$sql.= $ClsIng->inserta_notas(1,$cod_eva,$i,$preg,$tipo_evaluacion,$fecha,$usuario);
				}else if($i >5 and  $i <= 10){
					$sql.= $ClsIng->inserta_notas(2,$cod_eva,$i,$preg,$tipo_evaluacion,$fecha,$usuario);
				}else if($i >10 and  $i <= 15){
					$sql.= $ClsIng->inserta_notas(3,$cod_eva,$i,$preg,$tipo_evaluacion,$fecha,$usuario);
				}else if($i >15 and  $i <= 20){
					$sql.= $ClsIng->inserta_notas(4,$cod_eva,$i,$preg,$tipo_evaluacion,$fecha,$usuario);
				}		
			}
	...
	function Trae_Datos_Catalogo_inmediato($cat,$dep){
		$respuesta = new xajaxResponse();
	...
	function Trae_Datos_Catalogo_comte($cat,$dep){
		$respuesta = new xajaxResponse();
		$ClsPer = new ClsPersonal();
		// $respuesta->alert($cat);
		$result = $ClsPer->get_personal_usuario($cat);
