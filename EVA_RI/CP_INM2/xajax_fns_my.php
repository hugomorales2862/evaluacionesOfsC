...
$xajax->configure('decodeUTF8Input',true);  // para corregir 
	...
	function Grabar_evaluacion($cod_eva,$tipo_evaluacion,$preguntas,$usuario,$obs_inmediato,$obs_final,$obs,$autoevaluado){
		...
		if($tipo_evaluacion==4){
			$total_eva = $ClsIng->verifica_notas1($cod_eva,3,$autoevaluado);
		}
		if($tipo_evaluacion==19){
			$total_eva = $ClsIng->verifica_notas1($cod_eva,3,$autoevaluado);
		}

		if($tipo_evaluacion==22){
			$total_eva = $ClsIng->verifica_notas1($cod_eva,6,$autoevaluado);
		}

		if($tipo_evaluacion==27){
			$total_eva = $ClsIng->verifica_notas1($cod_eva,3,$autoevaluado);
		}
		if($total_eva > 0){
		...
		$sql = $ClsIng->update_evaluacion($cod_eva,$tipo_evaluacion,$obs_inmediato,$obs_final,$obs);
			...
			}elseif($tipo_evaluacion == 22){
				for($i = 1; $i <= 20; $i++){
					$preg = $preguntas[$i];
					if($i <= 5){
						$sql.= $ClsIng->inserta_notas(1,$cod_eva,$i,$preg,6,$fecha,$usuario);
					}else if($i >5 and  $i <= 10){
						$sql.= $ClsIng->inserta_notas(2,$cod_eva,$i,$preg,6,$fecha,$usuario);
					}else if($i >10 and  $i <= 15){
						$sql.= $ClsIng->inserta_notas(3,$cod_eva,$i,$preg,6,$fecha,$usuario);
					}else if($i >15 and  $i <= 20){
						$sql.= $ClsIng->inserta_notas(4,$cod_eva,$i,$preg,6,$fecha,$usuario);
					}
				}
			}elseif($tipo_evaluacion == 27){
				for($i = 1; $i <= 20; $i++){
					$preg = $preguntas[$i];
					if($i <= 5){
						$sql.= $ClsIng->inserta_notas(1,$cod_eva,$i,$preg,3,$fecha,$usuario);
