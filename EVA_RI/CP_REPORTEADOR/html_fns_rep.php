...
{
	$ClsPer = new ClsPersonal();
	$result = $ClsPer-> getAuditoria($dependencia, $comp_eva, $usuario);
	$salida = '';
	if ($result != "") {
		...
		$salida .= '<td width = "150px" 	align="center"><font size=2><b>	APROBO</b></font></td>';
...
function tabla_fi_aprobados($dependencia, $comp_eva, $usuario)
{
	$ClsPer = new ClsPersonal();
	...
	if ($result != "") {
		...
		$salida .= '<td width = "1px" 	align="center"> ANULAR</td>';
		$salida .= '</tr>';
		$salida .= '</thead>';
		$salida .= '<tbody class= "Sub-heading">';
		$cont = 0;
		foreach ($result as $row) {
			...
			if ($situacion == 4 or $situacion == 8 or $situacion == 13 or $situacion == 19  or $situacion == 22 or $situacion == 27 or $situacion == 35) {
				...
				$salida .= "<td align='center'>
								<a href='../CP_REP/REP_eva.php?eva=" . intval($eva_id) . "' target='_blank' class='btn btn-warning' title='Generar reporte final'>
									<i class='icon-file'></i>
								</a>
							</td>";

				$salida .= "<td align='center' bgcolor='#F5F6CE'>
								<a onclick='xajax_confirmacionParaEliminar(" . intval($eva_id) . ")' class='btn btn-danger' title='ANULAR'>
...
function excel_fi_aprobados($dependencia, $comp_eva, $usuario)
{
	$ClsPer = new ClsPersonal();
	$result = $ClsPer->get_oficiales_fin1($dependencia, $comp_eva, $usuario);
	$salida = '';
	if ($result != "") {
		$salida .= "<table border='1' cellspacing='0' cellpadding='5'>";
		$salida .= '<thead>';
		$salida .= '<tr>';
		$salida .= '<th width="75px" align="center" style="background-color: yellow;><font size="2"><b>No.</b></font></th>';
		$salida .= '<th width="75px" align="center" style="background-color: yellow;><font size="2"><b>EVALUACION</b></font></th>';
		$salida .= '<th width="75px" align="center" style="background-color: yellow;><font size="2"><b>CATALOGO</b></font></th>';
		$salida .= '<th width="150px" align="center" style="background-color: yellow;><font size="2"><b>GRADO</b></font></th>';
		$salida .= '<th width="350px" align="center" style="background-color: yellow;><font size="2"><b>NOMBRE COMPLETO</b></font></th>';
		$salida .= '<th width="150px" align="center" style="background-color: yellow;><font size="2"><b>APROBO</b></font></th>';
		$salida .= '<th width="150px" align="center" style="background-color: yellow;><font size="2"><b>SITUACION</b></font></th>';
...
function get_oficiales_data($dependencia, $comp_eva, $usuario)
{
	$ClsPer = new ClsPersonal();
	$result = $ClsPer->get_oficiales_fin1($dependencia, $comp_eva, $usuario);
	...
	if (!empty($result)) { // Verifica si $result no está vacío
		$cont = 0;
		foreach ($result as $row) {
			$cont++;
			$nombre = trim($row['PER_NOM1']) . " " . trim($row['PER_NOM2']) . " " . trim($row['PER_APE1']) . " " . trim($row['PER_APE2']);
			...
			// Determinar la descripción de la situación
			$situacion_desc = '';
			if (in_array($situacion, [4, 8, 13, 19, 22, 27, 35])) {
				$situacion_desc = 'PENDIENTE';
			} elseif (in_array($situacion, [28, 29, 30, 31, 32, 33])) {
				$situacion_desc = 'APROBADA';
			} elseif ($situacion == 34) {
				$situacion_desc = 'IMPUGNADO';
			}
			...
			$data[] = [
				...
				'Evaluacion' => $periodo,
				'Catalogo' => $catalogo,
				'Grado' => in_array($grado, [46, 59, 65, 73, 81, 88, 92, 93, 96, 97, 99, 40]) ? $gra_desc : $gra_desc . ' ' . $arm_desc,
				'Nombre Completo' => $nombre,
				'Aprobo' => $usuario,
				'Situacion' => $situacion_desc
