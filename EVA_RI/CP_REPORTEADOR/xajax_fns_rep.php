...
function confirmacionParaEliminar($cod_eva)
{
	...
	if ($cod_eva != "") {
		...
		$respuesta->script(
            ...
            '      <h5 class=\"modal-title\" id=\"myModalLabel\">Confirmacion</h5>' +
            '      <button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\">' +
            '        <span aria-hidden=\"true\">&times;</span>' +
            '      </button>' +
            '    </div>' +
            '    <div class=\"modal-body\">Esta seguro de eliminar esta evaluacion?</div>' +
            '    <div class=\"modal-footer\">' +
            '      <button type=\"button\" class=\"btn btn-secondary\" data-dismiss=\"modal\">Cancelar</button>' +
            '      <button type=\"button\" class=\"btn btn-primary\" onclick=\"xajax_Eliminar_evaluacion(" . intval($cod_eva) . ")\">Eliminar</button>' +
            '    </div>' +
            '  </div>' +
            '</div>'
		");
		$respuesta->script("$('#alert_modal').modal({backdrop:'static',keyboard:false, show:true});");
...
function Eliminar_evaluacion($cod_eva,$dep, $periodo, $usuario)
{
	$respuesta = new xajaxResponse();
	...
	if ($cod_eva != "") {
		...
		if ($rs == 1) {
			$respuesta->script("document.getElementById('alert_modal').innerHTML = '<div class=\"modal-dialog\" role=\"document\"><div class=\"modal-content\"><div class=\"modal-header\"><h5 class=\"modal-title\" id=\"myModalLabel\">Mensaje</h5><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button></div><div class=\"modal-body\">Eliminado Exitosamente</div><div class=\"modal-footer\"><button type=\"button\" class=\"btn btn-primary\" data-dismiss=\"modal\" aria-label=\"Close\" onclick=\"window.location.reload();\">Cerrar</button></div></div></div>';");
...
function Buscar_Vacios($dep, $periodo, $usuario)
{
	$respuesta = new xajaxResponse();
	$ClsPer = new ClsPersonal();
	$contenido = tabla_fi_aprobados($dep, $periodo, $usuario);
	$contenido2 = tablaPerosnalAprobo($dep, $periodo, $usuario);
	// $contenido_json = json_encode($contenido2);

	// $respuesta->alert($contenido2);
	$respuesta->assign("resultado", "innerHTML", $contenido);
	$respuesta->script("document.getElementById('btnDescargarExcel').style.display = 'block';");

	return $respuesta;
}


function Buscar_Vacios2($dep, $periodo, $usuario)
{
	$respuesta = new xajaxResponse();
	$ClsPer = new ClsPersonal();
	$contenido2 = tablaPerosnalAprobo($dep, $periodo, $usuario);
...
$xajax->register(XAJAX_FUNCTION, "Buscar_Vacios2");
...
$xajax->register(XAJAX_FUNCTION, "confirmacionParaEliminar");
