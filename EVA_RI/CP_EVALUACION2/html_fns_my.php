...
include_once("../html_fns.php");
	...
	function tabla_inmediato($dependencia,$comp_eva){
		$ClsPer = new ClsPersonal();
		$result = $ClsPer->get_oficiales_2comtes($dependencia,$comp_eva);
		if($result != ""){
			$salida.= "<div class='table-responsive'>";
			$salida.= "<table class='table table-condensed table-bordered table-hover'>";
			$salida.= '<thead>';
			...
			foreach($result as $row){
				...
				if ($grado == 46 || $grado == 59 || $grado == 65 || $grado == 73 || $grado == 81 || $grado == 88 || $grado == 92 || $grado == 93 || $grado == 96 || $grado == 97 || $grado == 99 || $grado == 40){
					...
					$salida.= "<td align = 'center'><a href = '../CP_INM1/Frm_mod.php?eva=".$eva_id."&sit=10' onclick = '' type = 'button' class='btn btn-default' title = 'Modificar borrador'><i class='icon-pencil'></i></a></td>";
					if($situacion == 5){
						$salida.= "<td align = 'center'><a href = '../CP_EVALUACION2/Frm_mia.php?eva=".$eva_id."&sit=6'  onclick = '' type = 'button' class='btn btn-info' title = 'Evaluar'><i class='icon-user'></i></a></td>";
					}elseif($situacion == 10){
						$salida.= "<td align = 'center'><a href = '../CP_EVALUACION2/Frm_mia.php?eva=".$eva_id."&sit=11'  onclick = '' type = 'button' class='btn btn-info' title = 'Evaluar'><i class='icon-user'></i></a></td>";
					}else{
						$salida.= "<td align = 'center'><a href = '../CP_EVALUACION2/Frm_mia2.php?eva=".$eva_id."&sit=40'  onclick = '' type = 'button' class='btn btn-info' title = 'Evaluar'><i class='icon-user'></i></a></td>";
					}
